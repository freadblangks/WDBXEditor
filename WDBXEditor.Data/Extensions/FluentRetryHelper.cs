...
namespace WDBXEditor.Data.Extensions
{
    public static class FluentRetryHelper
    {
        ...
        {
            return new Func<T>(() => RetryHelper.TryWithFailover<T>(baseLogic, failoverLogic, checkToFailover));
        }
        ...
        {
            return new Func<Task<T>>(() => RetryHelper.TryWithFailoverAsync<T>(baseLogic, failoverLogic, checkToFailover, cancellationToken));
        }
        ...
        {
            return new Func<T>(() => RetryHelper.BasicRetry<T>(func, retries, retryLogic, exceptionAggregator, waitLogic));
        }
        ...
        {
            return new Func<T>(() => RetryHelper.BasicRetry<T>(func, retries, retryLogic, exceptionAggregator));
        }
        ...
        {
            return new Func<Task<T>>(() => RetryHelper.BasicRetryAsync<T>(func, retries, retryLogic, exceptionAggregator, waitLogic, cancelToken));
        }
        ...
        {
            return new Func<Task<T>>(() => RetryHelper.BasicRetryAsync<T>(func, retries, retryLogic, exceptionAggregator, cancelToken));
        }
